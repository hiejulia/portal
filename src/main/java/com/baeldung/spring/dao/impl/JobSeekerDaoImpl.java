...
public class JobSeekerDaoImpl implements JobSeekerDao {
	 ...
	 */
	// filter jobs : JobPostingView jpv, List<?> jobIds
	@Override
	public List<?> filterJobs(JobPostingsView jpv, List<?> jobIds) {
		// boolean locationFlow = false l false
		...
		if (null != jpv.getLocation()) {// if job posting have position
			locationFlag = true;
			// concat : and jpv.location in location
	 ...
	 */
	// search jobs : searchTerm
	@Override
	public List<?> searchJobs(String searchString) {
		// add % searchTerm %
		searchString = "%" + searchString + "%";
		// process the search term before saving - check het src code -
		...
		// where : concat .jp.keywords like :searchTerm
...
// get user id from email : no ngoi mot minh no lam cai tro con meo gi day
